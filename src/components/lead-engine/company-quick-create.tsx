...
import { leadEngineFormOptionsAction } from "@/app/(app)/sales/lead-engine/actions";

/** "+ Firma"-Button mit Modal. Quellen werden beim Oeffnen geladen. */
export function CompanyQuickCreate({
  label = "Firma",
}: {
  label?: string;
}) {
  ...
  useEffect(() => {
    if (open && !sources) {
      leadEngineFormOptionsAction().then((r) =>
        setSources(r.ok && r.data ? r.data.sources : []),
      );
    }
  }, [open, sources]);
