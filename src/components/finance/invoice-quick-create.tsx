...
import { financeFormOptionsAction } from "@/app/(app)/finance/actions";

/** "+ Rechnung"-Button mit Modal. Kunden werden beim Oeffnen geladen. */
export function InvoiceQuickCreate({
  label = "Rechnung",
}: {
  label?: string;
}) {
  ...
  useEffect(() => {
    if (open && !clients) {
      financeFormOptionsAction().then((r) =>
